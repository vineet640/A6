...
class Keyboard:
    ...
    def __str__(self):
        ...
        """
        krows = []
        length = len(self.rows[0]) * 2 - 1
        for row in self.rows:
            cletters = []
            for letter in row:
                cletters.append(f"{self.colors[letter]}{letter}{NO_COLOR}")
            therow = " ".join(cletters)
            row_width = len(row) * 2 - 1
            empty = (length - row_width) // 2
            krows.append(" " * empty + therow)
        return "\n".join(krows)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            ...
            g_index_left.append(i)

    for i in g_index_left:
        guessed_character = guessed_word[i]
        if guessed_character in secretleft:
            feedback[i] = WRONG_SPOT_COLOR
            secretleft[secretleft.index(guessed_character)] = None
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    """
    word_family_dictionary = {}

    for secret_word in remaining_secret_words:
        feedback_colors = get_feedback_colors(secret_word, guessed_word)
        tuple1 = tuple(feedback_colors)

        if tuple1 not in word_family_dictionary:
            word_family_dictionary[tuple1] = []
        word_family_dictionary[tuple1].append(secret_word)

    wordfam = []
    for feedback_colors, words in word_family_dictionary.items():
        wordfam.append(WordFamily(feedback_colors, words))

    fams_sorted = fast_sort(wordfam)

    hardestfam = fams_sorted[0]

    return hardestfam.feedback_colors, hardestfam.words
